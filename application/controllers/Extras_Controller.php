<?php
class Extras_Controller extends CI_Controller
{
	...
	public function editPage()
	{
		...
		$js5 = base_url('assets/js/extras_content.js');
		...
		$view = array(
			'content' => strval($this->parser->parse('Contents/Extras', $data, TRUE)),
	...
	public function editAction($id)
	{
		...
		$data = array(
			'extra' => $this->Extras_Model->getExtraId($id)
		);

		$view = array(
			'content' => strval($this->parser->parse('Contents/Extra_edit', $data, TRUE)),
	...
	public function update()
	{
		...
		$config['upload_path']          = realpath(APPPATH . '../assets/images/extra_image');
		$config['allowed_types']        = 'jpg|png';

		$this->load->library('upload', $config);
		$this->upload->initialize($config);
		if (!$this->upload->do_upload('extra_image')) {
			$data = $this->input->post(array('id', 'extra_name', 'extra_icon', 'extra_caption', 'extra_contents'), true);
			$this->Extras_Model->updateExtra($data);
			$this->session->set_flashdata('success', 'Upload Success');
			redirect(base_url('/edit/extra'));
		} else {
			// print_r($_FILES('extra_image'));
			$file_name = $this->upload->data()['file_name'];
			$data = $this->input->post(array('id', 'extra_name', 'extra_icon', 'extra_caption', 'extra_contents'), true);
			$data['extra_image'] = base_url('assets/images/extra_image/'.$file_name);
			$this->Extras_Model->updateExtra($data);
			$this->session->set_flashdata('success', 'Upload Success');
			redirect(base_url('/edit/extra'));
		}
	...
	public function create()
	{
		...
		$view = array(
			'content' => strval($this->load->view('Contents/Extra_Create', '', TRUE)),
			'css' => '<link rel="stylesheet" href=' . $css1 . '><link rel="stylesheet" href=' . $css2 . '>',
			'js' => '<script src=' . $js1 . '></script><script src=' . $js2 . '></script><script src=' . $js3 . '></script><script src=' . $js4 . '></script><script src=' . $js5 . '></script><script src=' . $js6 . '></script><script src=' . $js7 . '></script>'
		);
		$main = $this->load->view('index', '', TRUE);
	...
	public function insert()
	{
		...
		if (!$this->upload->do_upload('extra_image')) {
			$data = $this->input->post(array('id', 'extra_name', 'extra_icon', 'extra_caption', 'extra_contents'), true);
			$this->Extras_Model->insertExtra($data);
			$this->session->set_flashdata('success', 'Upload Success');
			redirect(base_url('/edit/extra'));
		} else {
			// print_r($_FILES('extra_image'));
